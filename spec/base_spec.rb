  ...
  describe "the class" do
    ...
    it "should create a key from the right room" do
      ...
      ModelMock.room_key(t).should == 
        (Digest::SHA2.new << "ModelMock" << ts.to_s << NoDevent::Emitter.config["secret"]).to_s + ":" + ts
    end
    ...
    describe "with a custom room name" do
      ...
      it "should create a key from the right room" do
        ...
        ts = (t.to_f*1000).to_i
        ModelMock.room_key(t).should == 
          (Digest::SHA2.new << ModelMock.room << ts.to_s << NoDevent::Emitter.config["secret"]).to_s + ":" + ts
      end
    end  
  end
  ...
  describe "an instance of the class" do
    ...
    it "should create a key from the right room" do
      ...
      instance.room_key(t).should == 
        (Digest::SHA2.new << instance.room << ts.to_s << NoDevent::Emitter.config["secret"]).to_s + ":" + ts
    end
    ...
    describe "with a custom room name" do
      ...
      it "should create a key from the right room" do
        ...
        ts = (t.to_f*1000).to_i
        instance.room_key(t).should == 
          (Digest::SHA2.new << "otherRoom" << ts.to_s << NoDevent::Emitter.config["secret"]).to_s + ":" + ts
      end
      describe "#nodevent_create" do
        it "should emit to the right room" do
          $redis.should_receive(:publish).with("/nodevent", 
